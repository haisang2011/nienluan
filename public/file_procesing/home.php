...
<?php
		...
		while($row = $result->fetch_assoc()) {
			...
			array_push($data["intents"],json_decode('{"tag": "'."id_".$row["tt_ma"].'",
				...
				"responses": ["Có phải bạn đang tìm kiếm bảng tin này không? <a href=\"http://localhost/trangchu/'.$id_tintuc["tl_tenkhongdau"].'/'.$id_tintuc["lt_tenkhongdau"].'/'.$id_tintuc["tt_ma"].'\">Link</a>","Bạn đang tìm kiếm: <a href=\"http://localhost/trangchu/'.$id_tintuc["tl_tenkhongdau"].'/'.$id_tintuc["lt_tenkhongdau"].'/'.$id_tintuc["tt_ma"].'\">Link</a>"],
				"context_set": ""
			}',true));
            echo $str ."<br>";
